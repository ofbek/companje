...
void testApp::setup() {
    ...
    font.loadFont("Arial.ttf", 60, true, false, true);

    knor.loadSound("pig.wav");
    knor.setMultiPlay(true);
    
    text = "I am a flying pig";
}
        ...
        for(int j=0; j<obstacles.size(); j++) {            
            ...
            if (dist==0) {
                ...
                if (flock.boids[i].alive) {
                    ...
                    if (dist<5) { 
                        if (!knor.getIsPlaying()) knor.play();
                    }
...
void testApp::draw() {
    ...
    for(int i=0; i<flock.boids.size(); i++) {
        ...
        //string letter = ofToString(char((i%50)+'A'));
        
        string letter = ofToString(text.at(i % text.size()));
        
        ofFill();
        font.drawStringAsShapes(letter,0,0);
